...
for (s in seq_along(steps)) {
  ...
  # rename BEFORE building dat_tr/dat_te (and keep the same mapping)
  safe_names <- make.names(colnames(Xtr), unique = TRUE)
  ...
  # build formula with the renamed columns
  formula_tree <- as.formula(paste("Y ~", paste(safe_names, collapse = " + ")))

  fit <- tryCatch(tree(formula_tree, data = dat_tr),
                  error = function(e){ warning("top_", k, ": ", e$message); NULL })
  if (is.null(fit)) { tree_curve <- rbind(tree_curve, data.frame(TopGenes=k, Test_Acc=NA_real_)); next }

  tree_models[[s]] <- fit
  pred <- predict(fit, newdata = dat_te, type = "class")
  acc  <- mean(pred == dat_te$Y)
  cat("Top", k, "→ Test Acc:", sprintf("%.4f", acc), "\n")
  tree_curve <- rbind(tree_curve, data.frame(TopGenes = k, Test_Acc = acc))
}
```
