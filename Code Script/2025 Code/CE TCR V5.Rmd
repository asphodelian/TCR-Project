...
for (s in seq_along(steps)) {
  ...
  genes_k <- ranked[1:k]
  genes_k <- intersect(genes_k, intersect(names(train.data), 
                                          names(test.data)))
  ...
  # 4) Modeling frames (same columns, same order)
  dat_tr <- data.frame(Y = train.data$Y, Xtr[, safe_names, 
                                             drop = FALSE], 
                       check.names = FALSE)
  dat_te <- data.frame(Y = test.data$Y,  Xte[, safe_names, 
                                             drop = FALSE], 
                       check.names = FALSE)
