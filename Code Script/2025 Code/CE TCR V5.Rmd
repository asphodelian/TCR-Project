...
```{r}
safe_names <- make.names(genes5, unique = TRUE)
...
## 4) build formula safely and fit rpart classification tree with CV
...
## 5) prune at optimal cp (smallest xerror)
cpt <- fit$cptable
if (is.null(cpt) || nrow(cpt) == 0) {
  stop("rpart returned an empty cptable; try increasing cp or reducing predictors.")
}
...
## 6) predict on TEST and compute test error
pred <- predict(fit_pruned, newdata = dat_te, type = "class")
conf <- table(Predicted = pred, Actual = dat_te$Y)
test_error <- mean(pred != dat_te$Y)

cat("Top-5 α genes used:", paste(genes5, collapse = ", "), "\n")
cat("Optimal cp:", signif(cp_opt, 3), "\n")
print(conf)
cat("Test Error (misclassification rate):", sprintf("%.4f", test_error), "\n")
```
