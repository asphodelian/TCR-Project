...
if (tail(steps, 1) != length(ranked)) steps <- c(steps, length(ranked))
...
for (k in steps) {
  ...
  # Impute test from training medians (per feature)
  ...
  if (ncol(Xi_tr) == 0L) { glm_curve <- rbind(glm_curve, data.frame(TopGenes=k, Test_Acc=NA)); next }
  
  dat_tr <- data.frame(Y = train.data$Y, Xi_tr, check.names = FALSE)
  dat_te <- data.frame(Y = test.data$Y,  Xi_te, check.names = FALSE)
  
  fit <- tryCatch(glm(Y ~ ., data = dat_tr, family = binomial()), error = function(e) NULL)
  if (is.null(fit)) { glm_curve <- rbind(glm_curve, data.frame(TopGenes=k, Test_Acc=NA)); next }
  
  prob <- predict(fit, newdata = dat_te, type = "response")
  pred <- ifelse(prob > 0.5, 1, 0)
  acc  <- mean(pred == dat_te$Y)
  
  glm_curve <- rbind(glm_curve, data.frame(TopGenes = k, Test_Acc = acc))
}
glm_curve
