...
sampsize_grid <- c(0.5, 0.7, 1.0) # remove
# storage
...
for (k in steps) {
  ...
  # ---- Autotune by OOB error (no test leakage) ----
  best <- list(oob = Inf, fit = NULL, nt = NA_integer_, ns = NA_integer_, ss = NA_integer_)
  n_tr <- nrow(dat_tr)

  for (nt in ntree_grid) {
    for (ns in nodesize_grid) {
      for (sf in sampsize_grid) {
        ss <- max(1L, round(sf * n_tr))
        fit <- randomForest(
          form_bag, data = dat_tr,
          mtry = length(safe_names),  # bagging = all predictors
          ntree = nt,
          nodesize = ns,
          sampsize = ss,
          importance = FALSE
        )
        # OOB error from err.rate
        oob_err <- tail(fit$err.rate[, "OOB"], 1)
        if (!is.finite(oob_err)) {
          oob_err <- mean(fit$y != fit$predicted, na.rm = TRUE) # rare fallback
        }
        if (oob_err < best$oob) {
          best <- list(oob = oob_err, fit = fit, nt = nt, ns = ns, ss = ss)
        }
      }
    }
  }

  # Evaluate the best model on TEST (accuracy)
